...
namespace FCS.Utilities
{
    public class UserManagement 
    {
        ...
        public UserManagement(FCSContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        {
            SetSessionString("LastLogon", user.LastLogin.ToString());
            ...
            SetSessionInt32("LanguageId", user.LanguageId);
        }
        ...
        public void loadApplications(int UserId)
        {
            ...
            {
                var roleApplicationId = _context.Applications
              .Where(x => userRoles.Select(ur => ur.RoleId).Contains(x.Id))
              .Select(s => s.Id)
              .ToList();
